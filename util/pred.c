...
)
{
  ...
  /* ROOT output support.
     Example of calling: net-pred nz nets.net 201: / inputs.root:/dir/tree@1:1000,1,2,6,9,17 output.root:/dir/tree
     Option 'z' indicates output to ROOT file (output to stdout is not suppressed). Branches in output tree are not
     configurable. Currently supported only for 'n' option (mean of nets is stored). */
  int ROOT_output = 0;  // indicates whether output is set to ROOT file
  void *ROOT_file, *ROOT_tree;  // pointers to ROOT file and a tree
  
  char *op_z;
  
  if (argc == 7  &&  (op_z = strchr(argv[1], 'z')))
  {
    ROOT_output = 1;
    
    // Remove 'z' from the option list not to cause error in Neal's code
    int i;
    for (i = 0; i < strlen(op_z); i++)
      op_z[i] = op_z[i+1];
    
    char ROOT_file_name[256];
    char ROOT_dir_name[128];
    char ROOT_tree_name[128];
    
    // Reading ROOT file name (must be terminated with ':/')
    for (i = 0; !(argv[6][i] == ':'  &&  argv[6][i+1] == '/')  &&  argv[6][i] != '\0'; i++)
      ROOT_file_name[i] = argv[6][i];
    
    ROOT_file_name[i] = '\0';
    
    if (argv[6][i] == '\0')  // ':/' not found
    {
      fprintf(stderr, "ROOT file name must be of the form \"filename.root:/[dir1/[dir2/[...]]]tree\"\n");
      exit(1);
    }
    
    char *last_slash = strrchr(argv[6] + i + 2, '/');
    
    if (last_slash)  // last_slash == NULL if no in-file directory is specified
    {
      *last_slash = '\0';  // argv[6] is never user again, we can change it
      strcpy(ROOT_dir_name, argv[6] + i + 2);
      strcpy(ROOT_tree_name, last_slash + 1);
    }
    else
    {
      *ROOT_dir_name = '\0';
      strcpy(ROOT_tree_name, argv[6] + i + 2);
    }
    
    // Mimic ROOT file name in argv not to cause an error in Neal's code
    argc--;
    
    ROOT_file = CTFile_TFile(ROOT_file_name, "RECREATE");
    
    /*printf("File named %s is created\n", ROOT_file_name);*/
    
    if (ROOT_dir_name[0] != '\0')
    {
      char dir_name[128];
      strcpy(dir_name, ROOT_dir_name);  // we need to copy it since strtok changes its arguement string
      
      void *dir = CTFile_GetDirectory(ROOT_file, "");
      
      char *token = strtok(dir_name, "/");
      
      while (token)  // nested directories are allowed
      {
      	/*printf("Creating directory %s\n", token);*/
        dir = CTDirectory_mkdir(dir, token);
        token = strtok(NULL, "/");
      }
      
      CTFile_cd(ROOT_file, ROOT_dir_name);
      /*printf("cd to %s\n", ROOT_dir_name);*/
    }
    
    /*printf("Creating tree named %s\n", ROOT_tree_name);*/
    ROOT_tree = CTTree_TTree(ROOT_tree_name, "");
    /*CTTree_Write(ROOT_tree);*/
  }
...
"The m, d, D, q, and Q options are not currently implemented for count models\n"
  ...
  /* Adding ROOT branches. (Currently only option 'n' is supported.) */
  double ROOT_br_n;
  
  if (ROOT_output)
  {
    if (op_n)
      CTTree_Branch(ROOT_tree, "n", &ROOT_br_n, "n/D");
  }
  ...
  { 
    ...
    { 
      ...
      /* Putting value into ROOT branch. Currently only one target (binary classification) is supported. */
      ROOT_br_n = guess[j-1];
      /*printf(" (value %lf in branch)", ROOT_br_n);*/
    }
    ...
    /* This is the border of loop over test data entries. Fill ROOT tree. */
    CTTree_Fill(ROOT_tree);
  }
  
  /* Write ROOT tree and close the file. */
  CTFile_cd(ROOT_file, "");
  CTTree_Write(ROOT_tree);
  CTTree_Delete(ROOT_tree);
  CTFile_Close(ROOT_file);
