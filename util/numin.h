...
typedef struct
{
  ...
  /* It's same as 'index' but ordered and zero element appended. */
  int ifor[Max_items];	  /* What they're used for */
  /* Says what positions did elements from 'iused' occupy before ordering, i.e.
     in 'index', i.e. as they were given in 'argv'. iused[i] = index[ifor[i]] */
  ...
  int ROOT_input;   /* Indicates whether ROOT file is used as data input */
  void *ROOT_file;  /* Pointer to opened ROOT file */
  void *ROOT_tree;  /* Pointer to the ROOT tree containing data */
  double *ROOT_buffer;  /* Buffer to be associated with branches */
