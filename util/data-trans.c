...
/* Function 'data_trans' is modified to be tolerant to non-positive values when
 * a logarithmic transformation is needed. It returns shift * scale (so, the log
 * of data is set to zero).
 * -- Andrey Popov
 */
...
)
{
  ...
  { 
    if (d<=0) 
    { /*fprintf(stdout,
        "Warning: data to be logarithmically transformed is not positive\n");*/
      //exit(1);
