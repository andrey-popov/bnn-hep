...
/* Added information about the training cases weights into the
 * data_specifications structure. 'rescale_weights' = 0 indicates that
 * the weights should be taken as are, 1 means they should be rescaled in
 * such a way that they sum up to 'N_train', and 2 means that the signal's
 * and background's weights should sum up to 'N_train' / 2 independently.
 * Increased size of string buffers.
 * -- Andrey Popov
 */
...
typedef struct
{
  ...
  int has_weights;      /* Zero if the training cases are not weighted, one otherwise */
  int rescale_weights;  /* Indicates whether and which way the weights should be rescaled */

  char train_inputs[1000];	/* Source of training set input values */
  char train_targets[1000];	/* Source of training set target values */
  char train_weights[1000];  /* Source of training set weights */

  char test_inputs[1000];	/* Source of test set input values */
  char test_targets[1000];	/* Source of test set target values */
