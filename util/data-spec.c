...
/* Added support of the source of weights. This is an optional parameter that
 * takes place after the training targets specification. It must start with
 * "weights=" text.
 * -- Andrey Popov
 */
...
)
{
  ...
  // Read the location of the training weights
  if (*ap != 0 && strncmp(*ap, "weights=", 8) == 0)
  { ds->has_weights = 1;
    ds->rescale_weights = 3;  // default value
    strcpy(ds->train_weights, *ap + 8);
    ap++;
  }
  
  if (ds->has_weights && *ap != 0 && strncmp(*ap, "rescale_weights=", 16) == 0)
  { ds->rescale_weights = atoi(*ap + 16);
    ap++;
    
    if (ds->rescale_weights < 0 || ds->rescale_weights > 3)
    { fprintf(stderr, "rescale_weights = %d is illegal. Allowed values are 0, 1, 2, 3\n",
        ds->rescale_weights);
      exit(1);
    }
  }
