...
)
{
  ...
  for (l = 0; l<a->N_layers; l++)
  {
    ...
    { printf("\nHidden Layer %d to Hidden Layer %d Weights [%d]\n\n",l-1,l,g++);
      if (s!=0) printf("%5.5f",*s->hh_cm[l-1]);
      for (i = 0; i<a->N_hidden[l-1]; i++)
      { if (i>0) printf("\n");
        if (s!=0 && i>0) printf("     ");
        if (s!=0) printf(" %4.5f:",s->hh[l-1][i]);
    ...
    { printf("\nInput to Hidden Layer %d Weights [%d]",l,g++);
      ...
      printf("\n\n");
      if (s!=0) printf("%5.5f",*s->ih_cm[l]);
      j = 0;
      for (i = 0; i<a->N_inputs; i++)
      { if (flgs==0 || (flgs->omit[i]&(1<<(l+1)))==0)
        { if (j>0) printf("\n");
          if (s!=0 && j>0) printf("     ");
          if (s!=0) printf(" %4.5f:",s->ih[l][j]);
  ...
  { if (a->has_ho[l])
    { printf("\nHidden Layer %d to Output Weights [%d]\n\n",l,g++);
      if (s!=0) printf("%5.5f",*s->ho_cm[l]);
      for (i = 0; i<a->N_hidden[l]; i++)
      { if (i>0) printf("\n");
        if (s!=0 && i>0) printf("     ");
        if (s!=0) printf(" %4.5f:",s->ho[l][i]);
  ...
  { printf("\nInput to Output Weights [%d]",g++);
    ...
    printf("\n\n");
    if (s!=0) printf("%5.5f",*s->io_cm);
    j = 0;
    for (i = 0; i<a->N_inputs; i++)
    { if (flgs==0 || (flgs->omit[i]&1)==0)
      { if (j>0) printf("\n");
        if (s!=0 && j>0) printf("     ");
        if (s!=0) printf(" %4.5f:",s->io[j]);
...
{ 
  ...
  { if (i!=0)
    ...
    if (op) printf("%+6.5f",p[i]);
    else    printf("%+7.5f",p[i]);
  }
